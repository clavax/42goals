...
class Twitter extends Object
{
    ...
    const URL_VERIFY        = 'http://api.twitter.com/1/account/verify_credentials.json';
    ...
    public function __construct($key, $secret) 
    {
        ...
        if (isset($_COOKIE['tw_token'])) {
            $this->Session->tw_token = $_COOKIE['tw_token'];
            $this->Session->tw_secret = $_COOKIE['tw_secret'];
        }
        if (isset($this->Session->tw_token)) {
            $this->setAccessToken($this->Session->tw_token, $this->Session->tw_secret);
        }
    }
    ...
    {
        cookie::set('tw_token', $token, time() + 3600 * 24 * 30, '/', '');
        cookie::set('tw_secret', $secret, time() + 3600 * 24 * 30, '/', '');
    ...
    public function authorize($callback)
    {
        ...
        $this->Session->tw_request_token  = $request_token_info['oauth_token'];
        $this->Session->tw_request_secret = $request_token_info['oauth_token_secret'];
