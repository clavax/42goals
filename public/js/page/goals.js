...
document.addLoader(function () {
    if (document.body.id != 'goals-page') {
        return;
    }
    ...
    Observer.add('goal-edited', function (item) {
        ...
        var moved = ENV.tab && item.tab !== undefined && item.tab != ENV.tab;
    ...
    Observer.add('goal-removed', function (item) {
        var ind = -1;
        foreach(Data.goals, function (i, goal) {
            if (goal.id == item.id) {
                ind = i;
